...
class ConformerRNNTModule(LightningModule):
    ...
    def training_step(self, batch: Batch, batch_idx):
        ...
        batch_size = torch.tensor(batch.features.size(0), device=self.device, dtype=loss.dtype)
        batch_sizes = self.all_gather(batch_size).reshape(-1)

        self.log("Gathered batch size", batch_sizes.sum(), on_step=True, on_epoch=True)
        #loss *= batch_sizes.size(0) / batch_sizes.sum()  # world size / batch size
        loss *= batch_sizes.numel() / batch_sizes.sum()
        return loss
